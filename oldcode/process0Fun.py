...
Brate1=0x1C # bode rate for channel 1
...
def receive_data(matptr, calcul_batch, frequ):
	...
	while 1:
		...
		if TimeFlag or sRunFlag :
			...
			act_vehicle_num_ini = canDLL.VCI_Receive(USBCAN2,DEVICE_INDEX,CHANNEL1,byref(obj_vehicle_ini),max_vehicle_num_ini, 0)
			
			if act_vehicle_num_ini > 0:
				# -------------------------------get current time--------------------------------------
				if TimeFlag == 1:
					count = 0
					while count < act_vehicle_num_ini:
						if (obj_vehicle_ini[count].ID == 0x541): # ID of time channel
		...
		# ------------------------------------get data matrix--------------------------------------------
		max_sensor_num = canDLL.VCI_GetReceiveNum(USBCAN2,DEVICE_INDEX,CHANNEL0) # preview the number of data in cache 
		obj_sensor=(ZCAN_CAN_OBJ*ret_sensor_num)() # struct of data, timestamp, ID
		if max_sensor_num:
			act_sensor_num = canDLL.VCI_Receive(USBCAN2,DEVICE_INDEX,CHANNEL0,byref(obj_sensor),max_sensor_num, 0)
		...
		# print(Addr, min(2500, calcul_batch*frequ*6-Addr), ret_num, ret2)
			...
			if Addr >= calcul_batch*frequ*6:
				break

	# ???????????dbc_info = DBC_INFO(22, 0, 0, calcul_batch*frequ,0)
